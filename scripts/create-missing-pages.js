...
const treatments = [
  { name: "Forma RF", slug: "forma-rf", desc: "Radiofrequency treatment for skin rejuvenation and tightening" },
...
const pageTemplate = (name, slug, desc) => `"use client";
              ...
              {faqs.map((faq, index) => (
                <div key={index} className="bg-white rounded-xl overflow-hidden border border-gray-200">
                  ...
                  >
                    ...
                    <ArrowRight 
                      className={\`w-5 h-5 text-pastel-pink transition-transform \${openFaq === index ? 'rotate-90' : ''}\`} 
                    />
                  </button>
                  {openFaq === index && (
...
treatments.forEach(treatment => {
  ...
  // Write the page file
  fs.writeFileSync(filePath, pageTemplate(treatment.name, treatment.slug, treatment.desc));
  console.log(`Created: ${treatment.slug}/page.tsx`);
});

console.log(`\n✅ Created ${treatments.length} treatment pages!`);
